...
local function parseShopOpen(data)
    ...
    if offer then
      ...
      if panel.offerData.id ~= offerId or panel.offerData.cid ~= offer.cid or panel.offerData.count ~= offer.count or panel.offerData.price ~= offer.price then
        ...
        if offer.count > 1 then
          local totalText = "Total: " .. (offer.price * offer.count) .. "gp"
          panel:setTooltip(totalText)
          panel.offerItem:setTooltip(totalText)
        else
          local priceText = "Price: " .. offer.price .. "gp"
          panel:setTooltip(priceText)
          panel.offerItem:setTooltip(priceText)
        end
  ...
  -- Show seller shop message at their position when opening the shop
  do
    local msg = tostring(MainWindow.currentShop.message or Config.defaultShopMessage)
    local text = StaticText.create()
    text:addMessage(nil, MessageModes.NpcFrom, msg)
    g_map.addStaticText(text, seller:getPosition())
  end
...
local function parseShop(protocol, opcode, buffer)
  ...
  elseif evt == 'SHOP_NEARBY' then
    -- data: array of { id, info }
    local sellers = data or {}
    if #sellers == 0 then return end

    -- Try to display the message anchored at each seller's map position.
    for _, s in ipairs(sellers) do
      local creature = g_map.getCreatureById(s.id)
      if creature and creature:isPlayer() then
        local msg = tostring(s.info or Config.defaultShopMessage)
        -- StaticText placement (Lua-supported)
        local text = StaticText.create()
        text:addMessage(nil, MessageModes.NpcFrom, msg)
        g_map.addStaticText(text, creature:getPosition())
        -- Optional floating text on creature briefly
        creature:setText(msg, 'yellow')
        scheduleEvent(function()
          if creature then creature:clearText() end
        end, 2500)
      end
    end
    -- Also show a popup menu at cursor for quick open action.
    local menu = g_ui.createWidget('PopupMenu')
    for _, s in ipairs(sellers) do
      local label = tostring(s.info or 'Shop')
      menu:addOption(label, function() openShop(s.id) end)
    end
    menu:setGameMenu(true)
