...
function init()
  ...
  vocationBoxArcher    = spelllistWindow:getChildById('vocationBoxArcher')
   vocationBoxMastery   = spelllistWindow:getChildById('vocationBoxMastery')
  ...
  groupBoxMastery       = spelllistWindow:getChildById('groupBoxMastery')
  ...
  vocationRadioGroup:addWidget(vocationBoxMastery)
  ...
  groupRadioGroup:addWidget(groupBoxMastery)
...
function updateSpelllist()
  for i = 1, #SpelllistSettings[SpelllistProfile].spellOrder do
    ...
    if (not(filters.level) or info.level <= localPlayer:getLevel()) and (not(filters.vocation) or table.find(info.vocations, localPlayer:getVocation())) and (filters.vocationId == FILTER_VOCATION_ANY or table.find(info.vocations, filters.vocationId) or table.find(info.vocations, filters.vocationId)) and (filters.groupId == FILTER_GROUP_ANY or info.group[filters.groupId]) and (filters.premium == FILTER_PREMIUM_ANY or (info.premium and filters.premium == FILTER_PREMIUM_YES) or (not(info.premium) and filters.premium == FILTER_PREMIUM_NO)) then
...
function toggleFilter(widget, selectedWidget)
  if widget == vocationRadioGroup then
	...
	elseif boxId == 'vocationBoxArcher' then
	 ...
	 elseif boxId == 'vocationBoxMastery' then
      filters.vocationId = FILTER_VOCATION_MASTERY
    end
  elseif widget == groupRadioGroup then
	...
	elseif boxId == 'groupBoxMastery' then
      filters.groupId = FILTER_GROUP_MASTERY
    end
